...
import { TTargetContext } from "./types";
...
export function setSelectedTarget(elementId: number) {
  const target = utils.getElementById(elementId, getConfigs());

  selectedTarget.value = target;

  utils.scrollElementIntoView(target);
}
...
export function useTargetTypeName() {
  return computed(() => {
    ...
    return utils.getElementType(selectedTarget.value, getConfigs());
  });
}
